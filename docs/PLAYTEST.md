...
10. Build one ware to `×3`. Confirm its local shop card says `NEXT ATTUNES`, then buy or reforge a fourth copy. Before spending, the full-build preview must show `×3 → ×4 ATTUNED`, the doubled fourth-copy stat step, and the Attuned signature. After acceptance, Hero Stats must show the raw `×4`, `ATTUNED`, and its effective total; the same established battlefield signature should intensify without covering the hero or an enemy windup. A fifth and sixth copy must each return to one normal listed increment. Compare a `3/3` breadth build with a `4/2` specialist build and confirm the choice is understandable rather than compulsory.
