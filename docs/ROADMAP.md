...
- Resume from pushed and verified `0.1.33` and play Riftstalker through normal defense before changing another kit. The earlier readability, ranged-primary, gate, Wraith, economy, selling, item-breadth, and Cinder Wall checkpoints held, but Splitbolt still contradicted its approved execution promise: it granted three simultaneous generic violet blobs without waiting for a kill and read too similarly to Execution Volley.
- Replace only Riftstalker `E` with one authoritative lineage. Fire one `1.4`-radius seed toward the latest server-accepted aim when the windup resolves; let it sweep the complete path it actually travels, including a final partial step, and resolve up to four contacts in nearest physical order.
- Make the first confirmed enemy kill branch exactly once. Reserve two child IDs at cast, then create two `1.2`-radius forks at `±0.22` radians from the killed enemy. Each fork receives a fresh `2.2s`, inherits the seed's hit memory, can pierce four targets, and cannot branch again. A Rift Heart hit never creates forks.
- Keep the power ceiling bounded and knowable. Seed and forks retain the same `47` base damage, rank scaling, and creation-time Skill Power; their complete maximum remains `12` contacts and `564` base damage, exactly matching the old three-projectile ceiling while moving eight contacts behind successful execution.
- Preserve one stable multiplayer history. The seed's two reserved IDs retain the prior three-ID cast cadence even when no kill occurs, `splitStage` exposes only `seed` or `fork` publicly, private hit and trigger state stays server-side, disconnect leaves the same lineage active, and reconnect never duplicates or retroactively earns it.
- Replace the three orb-like blobs with one pale-violet directional needle and two slightly smaller branch darts. Use the compact Rift contact mark without the generic ability ring or particle burst; add no new effect kind, sound, shake, banner, meter, or parallel Riftstalker system. Execution Volley remains the larger immediate multi-arrow moment.
- Give Hero Stats the complete rule beside current damage: `4 PIERCE · KILL → 2×4`. Accessible text names the four-contact seed, first-kill branch, two four-contact forks, and nonrecursive limit without presenting the theoretical maximum as guaranteed output.
- Compare the shipped [three-blob baseline](playtest/splitbolt-before.jpg), final [single seed](playtest/splitbolt-seed.jpg), earned [two-fork branch](playtest/splitbolt-after.jpg), and [Hero Stats contract](playtest/splitbolt-stats.jpg). Every image is a real native `1280×720` render with empty warning/error diagnostics. The fork capture deliberately allows North to fall while a protected frame isolates both branches; it is mechanics evidence, not the balance run.
- Final normal-timing balance improves Riftstalker without erasing fragility. Across 30 runs, all Nexuses survive, `7` are clean, `26` downs occur, `15` gates remain intact, mean gate health is `101.8`, mean kills are `190.7`, and mean Nexus health is `784.47`. The shipped `0.1.33` cohort recorded the same `7` clean runs but `28` downs, `89.37` mean gate health, and `186.87` mean kills.
- Across 100 final runs, all Nexuses survive, `29` are clean, `86` downs occur, `43` gates remain intact, mean gate health is `87.24`, mean kills are `191.14`, and mean Nexus health is `787.16`. `1,480/1,617` casts earn the branch (`91.53%`). The shipped cohort recorded `23` clean runs, `102` downs, and `75.65` mean gate health.
- Typecheck, `223` tests with `13,336` assertions across `30` files, production asset `main-cf691b832513e9b6.js` at `640,698` bytes, fixed-asset `404` smoke, focused Splitbolt authority and reconnect coverage, and real four-client WebSocket convergence pass. All clients agree on one seed, the two reserved fork IDs, earned `seed`/`fork` stages, damage, geometry, and shared simulation state with `tickSpread: 0`.
- Feature commit [`c926f13`](https://github.com/FabienGreard/x-hero-siege/commit/c926f1367a9a6c2f9bf812f2e532dc552d69ba24) reached `origin/master`; [Pages run `29172338659`](https://github.com/FabienGreard/x-hero-siege/actions/runs/29172338659) succeeded. The [public companion](https://fabiengreard.github.io/x-hero-siege/) rendered `0.1.34` at `1280×720` with no horizontal overflow or warning/error diagnostics, and served the exact `1280×720` single-seed evidence at SHA-256 `71dde990e33da5e6cbec968e636c99c366c28c52e51ecf24173ac3817fee3b91`.

**Review evidence carried forward:** Splitbolt now differs from Execution Volley through one readable execution seed and an earned first-kill branch rather than another immediate group of projectiles. Riftstalker remains the fragile ranged champion, but normal cohorts show fewer downs, more kills, and stronger gate outcomes without increasing its old maximum output. Gravebinder's safety is still distributed across Soul Scythe, protection, and horde-derived effects; that broader systemic sustain remains the leading unresolved champion gap.

**Technical risk carried forward:** gameplay and presentation still share one RNG stream and one `idCounter`, so old and new cohorts remain broad build-level evidence rather than paired-seed causal attribution. Splitbolt now owns a focused swept-lineage path beside ranged-primary sweeping; keep collision-order and terminal-step parity covered if those paths are consolidated later. Any future playable-host rollout must still promote compatible server and hashed client bytes together.

**Next-cycle nomination:** begin from pushed and verified `0.1.34`, then replay all four champions with special attention to Gravebinder's ordinary Soul Scythe, protection, and summon interactions. If systemic sustain remains the highest-leverage player-visible weakness, choose one bounded kit rule from that played evidence rather than precommitting to a scalar nerf, rewriting the whole class, or adding another item.

## `0.1.33` — Draw the Line

**Status:** implemented, pushed, and locally verified on 2026-07-11; companion site deployed and live-render verified; playable game deployment pending a configured Bun/WebSocket host and the remaining public-host gates.

- Resume from locally verified `0.1.32` and play all four champions before changing another system. The earlier ranged-primary, screen, gate, Wraith, economy, selling, and item-breadth checkpoints held, but Ashcaller's `Cinder Wall` still contradicted its approved promise: five simultaneous generic circles performed one immediate area hit instead of leaving placed fire that could control a lane.
- Replace only Ashcaller `E` with one server-owned persistent line. Sample a committed center segment from `3.2 → 16` world units, retain the established rank-width growth from a `3.2` half-width, persist for `2s`, and apply the established `2s` slow.
- Keep damage bounded and knowable. Every present enemy takes the current cast's canonical damage at most once, a later entrant can be caught once before expiry, re-entry never repeats damage or refreshes slow, and the Rift Heart can be struck once. Apply present contacts as the cast lands before enemies act.
- Sample damage, rank geometry, origin, direction, and Skill Power at creation. Later Focus purchases, sales, reforges, rank changes, movement, or aim changes never rewrite the existing wall. A disconnected Ashcaller leaves the same authoritative wall alive through the established reservation, and reconnect restores its original visible ID without duplication.
- Replace five additive fire rings and their burst aftermath with one low translucent rounded line that preserves the playable footprint while allowing floor, demons, objective state, and the Ashcaller to remain readable. It uses restrained ember motion and adds no ring, particle burst, shake, banner, HUD meter, or sound.
- Preserve the previous presentation cadence with one visible two-second `cinder_wall` record plus four invisible one-second companion records. All four peers receive the same visible ID, midpoint, half-width, direction, and remaining time; companion records never enter the renderer or create removal feedback.
- Give the current-build surface the whole sentence. Cinder Wall retains canonical per-target damage, while its full-width Hero Stats row states `2S WALL · ONCE/TARGET`; accessible text names the same duration and per-cast limit without estimating total pack output.
- Compare the shipped [five-ring baseline](playtest/cinder-wall-before.jpg), final [persistent line](playtest/cinder-wall-after.jpg), and [full-width Hero Stats rule](playtest/cinder-wall-stats.jpg). Every image is a native `1280×720` fresh-client render; the rounded low-opacity wall preserves floor and enemy silhouettes, all targeted text fits, the viewport stays exact, and warning/error diagnostics are empty.
- Final normal-timing balance remains viable without claiming parity. Across 30 Ashcaller runs, `29` are clean, one down occurs, `29` gates remain intact, mean gate health is `243.63`, mean kills are `201.33`, and every Nexus survives. Across 100 final runs, Ashcaller records `96` clean runs, four downs, `97` intact gates, `240.07` mean gate health, and `200.89` mean kills; the prior `0.1.32` broad cohort recorded `70` clean, `31` downs, `78` intact, and `165.67` mean gate health.
- The all-hero 100-run comparison keeps Warden and Gravebinder at `100` clean runs with pristine `260` gates, while Riftstalker remains the leading difficulty at `23` clean runs, `102` downs, and `75.65` mean gate health. Gravebinder's systemic sustain is still unresolved; Cinder Wall does not tune either class.
- Typecheck, `208` tests with `13,014` assertions across `29` files, production asset `main-593e248fbf41e291.js` at `639,756` bytes, fixed-asset `404` smoke, and a real four-client WebSocket convergence run pass with `tickSpread: 0`, one shared visible wall, and four shared companion records.
- Feature commit [`d0f7294`](https://github.com/FabienGreard/x-hero-siege/commit/d0f7294d11a7cd0a3ab95304fedc370e4a10b4fc) reached `origin/master`; [Pages run `29169727733`](https://github.com/FabienGreard/x-hero-siege/actions/runs/29169727733) succeeded. The [public companion](https://fabiengreard.github.io/x-hero-siege/) rendered `0.1.33` at `1280×720` with no horizontal overflow or warning/error diagnostics, and served the exact `1280×720` live-contact evidence at SHA-256 `421f2d60ebd4a7bc2f803c0a40bcd2d25d9f0eed1daa5aab85065419907370df`.

**Review evidence carried forward:** Cinder Wall now gives Ashcaller a real placed-fire identity rather than another immediate circle, and the earlier Armory, screen, ranged-primary, gate, and Wraith promises remain intact. Riftstalker is again the highest player-visible combat difficulty. Gravebinder still combines systemic sustain sources strongly enough to require a separate played diagnosis rather than a token Soul Scythe scalar change.

**Technical risk carried forward:** gameplay and presentation still share one RNG stream and one `idCounter`. The old and new 100-run cohorts are broad build-level evidence, not paired-seed causal attribution; split both streams together before exact cross-build seed deltas become a tuning contract. New effect kinds also require fresh hashed clients, so any future playable-host rollout must promote compatible server and client bytes together.

**Next-cycle nomination:** begin from pushed and verified `0.1.33`, then replay Riftstalker through normal defense, first purchase, and completed-build windows. If its extreme downs and gate loss remain the leading rendered weakness, ship one bounded Riftstalker promise checkpoint rather than buffing every ranged skill or adding another ware. Keep Gravebinder's broader sustain audit explicit and separate.
