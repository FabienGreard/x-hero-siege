...
- **Splitbolt:** piercing shot whose first confirmed kill branches off two more bolts.
...
- **Cinder Wall:** a persistent server-owned burning line that catches each target once per cast.
...
- Hold left mouse for the primary attack; `Q`, `E`, and `R` trigger active abilities; `F` triggers the ultimate. `B` browses a physical shop only while in range, with visible `1`–`3` number shortcuts for its wares; `X` enters exact-slot selling only inside that local panel. There is no separate Space action or global shop menu.
...
- The verified local shop layer is the northwest Ironbound Forge plus the northeast Veilglass Reliquary. Their five inexhaustible 60-gold wares create three martial-survival answers at the Forge—Basic Damage, Move Speed, and Max Health—and two ability answers at the Reliquary—Skill Power and Cooldown Speed. Either shop buys one reviewed exact equipped slot for 30 gold, and a full build atomically reforges a slot for the 30-gold net difference. Repeatable defense rewards of `1/1/3` gold and preserved `35`-gold Gatebreaker and `6`-gold Rift Guard payoffs guarantee one defining choice while making later build completion and reshaping performance-dependent. North's equal-cost left/right retreat makes the inventory choice spatial without making solo defense depend on a long diagonal trip. East and West naturally favor different vendors; South is intentionally underserved rather than receiving global access.
...
- Cinder Wall samples its origin, committed direction, rank-scaled width, damage, and Skill Power when the cast resolves. Its center line begins `3.2` world units from the Ashcaller and ends at `16`; the rank-one `3.2` half-width grows by eight percent of its base for each later rank. The authoritative wall burns for `2s`, applies the established `2s` slow, damages each enemy at most once per cast, admits late entrants, and can strike the Rift Heart once. Hero Stats keeps current damage beside the full-width `2S WALL · ONCE/TARGET` behavior. One restrained translucent rounded line carries the playable footprint without rings, burst particles, or a new sound; four invisible one-second companion records preserve the established five-effect presentation cadence.
- Splitbolt fires one server-owned `1.4`-radius seed toward Riftstalker's latest accepted aim when the cast resolves. It sweeps every segment it actually travels, including the final partial step, and can strike up to four targets. Its first confirmed enemy kill branches off exactly two fresh `2.2s`, `1.2`-radius forks at `±0.22` radians; each fork can strike four targets, inherits the seed's hit memory, and can never branch again. The seed and both forks keep the cast-time rank and Skill Power damage, while two child IDs reserved with the seed make the lineage stable through multiplayer snapshots and reconnect. At base power the complete contact budget remains `12 × 47 = 564`, exactly matching the old three-projectile ceiling. Hero Stats states `4 PIERCE · KILL → 2×4`, and restrained directional seed, fork, and compact-contact reads replace three immediate generic violet blobs without adding a ring, burst field, sound, or another Riftstalker system.
- Gateward Plate grants exactly `+15 Max Health` per effective copy and uses the established fourth-copy Attunement arithmetic. Buying, selling, or reforging Max Health preserves current health percentage, so the ware changes capacity without becoming a heal or damage event. It never grants armor, regeneration, a proc, an active, or scaling to Soul Scythe, Bone Ward, Wraith Host, or any other champion ability.
...
- Heroes have exactly six unrestricted equipment slots: any item or duplicate may occupy any slot. Hero Stats groups duplicate copies into named stacks with canonical total effects, and each local shop discloses its current owned count before purchase, sale, or reforge. Selling and reforging must name one exact occupied slot and its complete current-to-projected consequence before confirmation. Keep the catalog curated and buy/equip decisions concise; deepen build tags, duplicate ranks, and one earned evolution at a time only after rendered play proves the prior layer. Broad loot catalogs, recipe trees, crafting, permanent progression, inventory grids, and automation remain deferred.
...
- A full-build reforge must expose its exact champion-specific consequence before commitment: outgoing and incoming effects, current-to-projected stack counts, every changed Hero Stat, the resulting dominant battlefield signature, and `60 − 30 = 30` gold net arithmetic. It remains one atomic server transaction so the six-slot build never exposes an intermediate hole. A standalone sale exposes the same outgoing consequences plus its open slot and `+30` wallet result. The projection uses canonical equipment and stat derivation, while the live Hero Stats surface remains authoritative and unchanged until the server accepts either trade.
