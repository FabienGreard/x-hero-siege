...
- Hold left mouse for the primary attack; `Q`, `E`, and `R` trigger active abilities; `F` triggers the ultimate. `B` browses a physical shop only while in range, with visible number shortcuts for its wares; `X` enters exact-slot selling only inside that local panel. There is no separate Space action or global shop menu.
