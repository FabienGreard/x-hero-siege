...
Version `0.1.11` is deliberately small: one 5–10 minute run that proves readable combat, party-sized lane defense, direct action-bar progression, truthful cooperative gold whose purchasing power carries weight, two physical shops with distinct run-only wares, a readable six-slot build, full-build reshaping, one pressure spike, and one boss payoff.

![The Warden inspects grouped equipment totals beside Ironbound Forge cards showing how many copies are already owned](docs/playtest/read-the-build-shop.jpg)
