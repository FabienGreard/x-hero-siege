...
Version `0.1.16` is deliberately small: one 5–10 minute run that proves readable combat, party-sized lane defense, direct action-bar progression, truthful cooperative gold whose purchasing power carries weight, two physical shops with distinct run-only wares, a readable and visibly resonant six-slot build with one meaningful duplicate threshold, knowable ordinary purchases and exact full-build reshaping, one pressure spike, and one boss payoff.

![The Warden sees the exact fourth Tempered Edge result before spending at the Ironbound Forge](docs/playtest/know-next-copy-after.jpg)
