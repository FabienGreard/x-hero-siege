...
Splitbolt now fulfills Riftstalker's approved execution promise. `E` releases one `1.4`-radius seed toward the latest accepted aim, sweeps the full path it actually travels, and can pierce four targets. Its first confirmed enemy kill branches off exactly two smaller bolts at `±0.22` radians; each receives a fresh `2.2s`, can pierce four targets, inherits the seed's hit memory, and cannot branch again. All three retain `47` base damage plus the rank and Skill Power sampled at cast time, so the maximum stays at the old `12` contacts and `564` base damage while eight contacts must now be earned. Hero Stats states `4 PIERCE · KILL → 2×4`, public snapshots identify the seed and forks across reconnect, and thin directional darts with compact contacts replace three immediate orb-like blobs without adding another ring, burst field, or sound.

Cinder Wall now fulfills the Ashcaller's approved placed-fire promise. The committed cast samples one authoritative line from `3.2` to `16` world units, burns for `2s`, widens with rank from a `3.2` half-width, and slows each enemy it catches for `2s`. A target can take the cast's damage only once, but demons entering later remain vulnerable until the wall fades. Skill Power is sampled when the line is created, the Rift Heart can be struck once, and reconnecting clients recover the same wall identity instead of creating another. Hero Stats joins current damage to the full-width `2S WALL · ONCE/TARGET` rule. One translucent rounded ground read replaces five generic fire rings without burst particles or a new sound.
