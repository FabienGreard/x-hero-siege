...
Version `0.1.12` is deliberately small: one 5–10 minute run that proves readable combat, party-sized lane defense, direct action-bar progression, truthful cooperative gold whose purchasing power carries weight, two physical shops with distinct run-only wares, a readable and visibly embodied six-slot build, full-build reshaping, one pressure spike, and one boss payoff.

![The Warden wears Tempered Edge's warm blade corona after a full-build replacement](docs/playtest/wear-the-build-edge.jpg)
