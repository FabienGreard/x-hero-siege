...
Version `0.1.14` is deliberately small: one 5–10 minute run that proves readable combat, party-sized lane defense, direct action-bar progression, truthful cooperative gold whose purchasing power carries weight, two physical shops with distinct run-only wares, a readable and visibly embodied six-slot build with one meaningful duplicate threshold, exact full-build reshaping, one pressure spike, and one boss payoff.

![The Forge shows the Warden that a fourth Tempered Edge will Attune the stack before the full-build reforge](docs/playtest/attune-the-fourth-preview.jpg)
