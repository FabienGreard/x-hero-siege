...
describe("equipment build summaries", () => {
  ...
  test("duplicates are grouped in first-equipped order with their total effects", () => {
    ...
    expect(summarizeEquipment(equipment)).toEqual([
      { itemId: "runebound_focus", count: 3, effectiveCount: 3, totalEffectLabel: "+45% Skill Power" },
      { itemId: "tempered_edge", count: 2, effectiveCount: 2, totalEffectLabel: "+40% Basic Damage" },
      { itemId: "quickening_sigil", count: 1, effectiveCount: 1, totalEffectLabel: "+15% Cooldown Speed" },
    ]);
  });
